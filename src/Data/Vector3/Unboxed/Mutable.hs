module Data.Vector3.Unboxed.Mutable
  ( MVector3(..)
  , IOVector3
  , V.Unbox
  , new
...
new :: (PrimMonad m, V.Unbox a) => Int -> Int -> Int -> m (MVector3 x y z (PrimState m) a)
new num_Xs num_Ys num_Zs =
  MVector3 num_Ys num_Zs <$> V.unsafeNew (num_Xs * num_Ys * num_Zs)
{-# INLINE new #-}

replicate :: (PrimMonad m, V.Unbox a) => Int -> Int -> Int -> a -> m (MVector3 x y z (PrimState m) a)
replicate num_Xs num_Ys num_Zs a =
  MVector3 num_Ys num_Zs <$> V.replicate (num_Xs * num_Ys * num_Zs) a
{-# INLINE replicate #-}

replicateM :: (PrimMonad m, V.Unbox a) => Int -> Int -> Int -> m a -> m (MVector3 x y z (PrimState m) a)
replicateM num_Xs num_Ys num_Zs a =
  MVector3 num_Ys num_Zs <$> V.replicateM (num_Xs * num_Ys * num_Zs) a
{-# INLINE replicateM #-}

--
size :: V.Unbox a => MVector3 x y z s a -> (Int, Int, Int) -- (xs, ys, zs)
size MVector3{..} =
  (V.length mvector `div` num_Ys `div` num_Zs, num_Ys, num_Zs)
{-# INLINE size #-}

read :: (PrimMonad m, V.Unbox a, Enum x, Enum y, Enum z)
...
write :: (PrimMonad m, V.Unbox a, Enum x, Enum y, Enum z)
  => MVector3 x y z (PrimState m) a
  -> x -> y -> z -> a -> m ()
...
modify :: (PrimMonad m, V.Unbox a, Enum x, Enum y, Enum z)
  => MVector3 x y z (PrimState m) a
  -> x -> y -> z -> (a -> a) -> m ()
...
imapM_ :: (PrimMonad m, V.Unbox a, Enum x, Enum y, Enum z)
  => (a -> x -> y -> z -> m ())
  -> MVector3 x y z (PrimState m) a
  -> m ()
